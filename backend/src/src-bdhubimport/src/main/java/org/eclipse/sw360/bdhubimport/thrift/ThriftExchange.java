...
public class ThriftExchange {
    ...
    public Optional<List<License>> searchLicenseByWsName(String wsName) {
        return getFilteredLicenseList(license ->
                        license.isSetExternalIds() && CommonUtils.nullToEmptyString(license.getExternalIds().get(TranslationConstants.BDHUB_ID)).equals(wsName),
                "wsId=[" + wsName + "]:"
        );
    }

    boolean projectExists(String projectName, User user) throws TException {
        List<Project> accessibleProjects = getAccessibleProjectsSummary(user);
        if (hasAccessibleProjectWithBDHubName(projectName, accessibleProjects)) {
            LOGGER.info("Project to import was already in SW360 with name: " + projectName);
            return true;
        }
        return false;
    }
    ...
    private boolean hasAccessibleProjectWithBDHubName(String projectName, List<Project> accessibleProjects) {
